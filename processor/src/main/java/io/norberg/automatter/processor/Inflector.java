...
/**
 * Transforms words to singular, plural, humanized (human readable), underscore, camel case, or
 * ordinal form. This is inspired by the <a
...
class Inflector {
   ...
   * </pre>
   *
   * <p>Note that if the {@link Object#toString()} is called on the supplied object, so this method
  ...
  /**
   * By default, this method converts strings to UpperCamelCase. If the <code>uppercaseFirstLetter
   * </code> argument to false, then this method produces lowerCamelCase. This method will also use
  ...
  public String camelCase(
      String lowerCaseAndUnderscoredWord, boolean uppercaseFirstLetter, char... delimiterChars) {
    ...
    if (uppercaseFirstLetter) {
      ...
      // Replace any extra delimiters with underscores (before the underscores are converted in the
      // next step)...
  ...
  /**
   * Makes an underscored form from the expression in the string (the reverse of the {@link
   * #camelCase(String, boolean, char[]) camelCase} method. Also changes any characters that match
   * the supplied delimiters into underscore.
   *
   * <p>Examples:
   *
   * <pre>
  ...
  /**
   * Returns a copy of the input with the first character converted to uppercase and the remainder
   * to lowercase.
   *
   ...
   * supplied removable tokens. Like {@link #titleCase(String, String[])}, this is meant for
  ...
  public String titleCase(String words, String... removableTokens) {
    ...
    result =
        replaceAllWithUppercase(
            result, "\\b([a-z])", 1); // change first char of each word to uppercase
    return result;
  }
  ...
  public void addIrregular(String singular, String plural) {
    requireIsNotEmpty(singular, "singular rule");
    requireIsNotEmpty(plural, "plural rule");
  ...
  private void requireIsNotEmpty(String s, String message) {
