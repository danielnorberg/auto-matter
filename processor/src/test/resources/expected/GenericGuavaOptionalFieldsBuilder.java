...
public final class GenericGuavaOptionalFieldsBuilder<T> {
  ...
  public static <T> GenericGuavaOptionalFieldsBuilder<T> from(
      GenericGuavaOptionalFields<? extends T> v) {
    return new GenericGuavaOptionalFieldsBuilder<T>(v);
  }

  public static <T> GenericGuavaOptionalFieldsBuilder<T> from(
      GenericGuavaOptionalFieldsBuilder<? extends T> v) {
    return new GenericGuavaOptionalFieldsBuilder<T>(v);
  }
  ...
  private static final class Value<T> implements GenericGuavaOptionalFields<T> {
    ...
    private Value(@AutoMatter.Field("foo") Optional<T> foo,
        @AutoMatter.Field("bar") Optional<T> bar) {
      if (foo == null) {
        throw new NullPointerException("foo");
      }
      this.foo = foo;
